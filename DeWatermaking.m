...
Im = imread("IMGPretoeBrancoParaDW.tiff");  %ler a imagem de entrada

% Resize the image
img_resized = imresize(Im, [512 512]);
...
f1 = figure(1);
f1 = figure('Name', 'Imagem principal Para DW'); 
imagesc(Im);
...
%DCT => dy1(1:rm,1:cm) = dy1(1:rm,1:cm) - g*m;
...
figure('Name','Componente sem Marca dágua'), imshow(dx1); %displaying each component of the image after watermarking (Figura 3)
...
figure(2);
figure('Name','Inversa já sem marca dágua'), imagesc(I_sdct_inversa/255);
